...
const Navbar = () => {
    ...
    return (
        <header>
            ...
            <div className={`absolute top-[48px] w-full flex justify-center items-center bg-transparent transition-all duration-300 lg:flex  ${menu ? "delay-1000 opacity-0" : "opacity-100"}`}>
                <div className="w-full h-full flex justify-between items-center max-w-6xl px-2 lg:px-0">
                    ...
                    <div className="hidden lg:flex gap-8">
                        <div className={`flex items-center gap-1 p-2 ${location.pathname == "/" ? "text-thirty" : ""}`}>
                            <Link href={"/"} className="hover:text-thirty transition-all duration-200 p-2">Home</Link>
                        </div>
                        <div className="relative flex items-center gap-1 p-2 ">
                            ...
                            <div className="hidden peer-hover/1:absolute peer-hover/1:block hover:absolute hover:block top-12 bg-white w-72 rounded z-20 peer/2">
                                <Link href={"/sejarah"} className="my-4 ml-6 w-full block hover:text-thirty transition-all duration-200">Sejarah</Link>
                                <Link href={"/visi-dan-misi"} className="my-4 ml-6 w-full block hover:text-thirty transition-all duration-200">Visi dan Misi</Link>
                        ...
                        <div className="relative flex items-center gap-1 p-2">
                            ...
                            <div className="hidden peer-hover/1:absolute peer-hover/1:block hover:absolute hover:block top-12 bg-white w-72 rounded z-20 peer/2">
                                {major.map((data, i) => {
                                    return (
                                        <Link key={i} href={data.path} className="my-4 ml-6 w-full block hover:text-thirty transition-all duration-200">{data.name}</Link>
                                    )
                                })}
                            </div>
                            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" className="peer-hover/1:rotate-180 peer-hover/2:rotate-180 transition-all duration-500" fill="currentColor" viewBox="0 0 256 256"><path d="M216.49,104.49l-80,80a12,12,0,0,1-17,0l-80-80a12,12,0,0,1,17-17L128,159l71.51-71.52a12,12,0,0,1,17,17Z"></path></svg>
                        ...
                        <div className="flex items-center gap-1 p-2">
                            <Link href={"/faqs"} className="hover:text-thirty transition-all duration-200 p-2">Gallery</Link>
                        </div>
                        <div className="flex items-center gap-1 p-2">
